...
const HARDER = `rhythm quartz jukebox syndrome awkward whiskey lymph blizzard sphinx crypt glyph khaki vodka waltz zephyr jinx mnemonic phlegm queue rendezvous silhouette turquoise vacuum wharf xylophone yacht zealous asymmetry bureaucracy conscience embarrass fluorescent guarantee hierarchy inoculate liaison maintenance necessary occurrence perseverance questionnaire restaurant separate threshold unanimous vengeance`.split(' ');

const PUNCTUATED = `don't it's they're we've I'd shouldn't couldn't; "quoted," (parenthetical) — dash, semi; colon: 42% $19.99 #tag &amp @user *star ~tilde /slash \\back |pipe [bracket] {brace} <angle> +plus =equals`.split(' ');
